<template>
    <ion-list class="navbar-nav iq-main-menu" id="sidebar-menu">
        <ion-item lines="none">
        ...
        <ion-item-group>
            ...
            <ion-item v-for="(item, index) in items" :key="index" :router-link="item.link">
                <ion-icon :icon="item.icon" slot="start"></ion-icon>
                <ion-label>{{ item.name }}</ion-label>
            </ion-item>
        </ion-item-group>
        <!-- <ion-item v-for="items in navItem" :key="items" :class="checkActive(items.child) ? 'active' : ''">
            <ion-label @click="toggleSubMenu(items.name)">
                <ion-icon :icon="items.icon" slot="start"></ion-icon>
                <span class="item-name">{{ items.name }}</span>
                <ion-icon name="chevron-forward-outline" slot="end"></ion-icon>
            </ion-label>
            <ion-list>
                <ion-item v-for="childItem in items.child" :key="childItem"
                    :class="checkActive(childItem.url) ? 'active' : ''">
...
<script>
import { IonList, IonItem, IonIcon, IonLabel, IonItemDivider, IonItemGroup } from '@ionic/vue';
import { chevronDownOutline, ellipse } from 'ionicons/icons';
export default {
    ...
    data() {
        ...
        return {
            ...
            chevronDownOutline,
            items: [
                { name: 'Dashboard', link: 'http://one.bma.edu.ph/administrator/dashboard?_academic=MTA=', icon: 'ellipse' },
                { name: 'Semestral Clearance', link: 'http://one.bma.edu.ph/administrator/semestral-clearance?_academic=MTA=', icon: 'ellipse' },
                { name: 'Students', link: 'http://one.bma.edu.ph/administrator/students?_academic=MTA=', icon: 'ellipse' },
                { name: 'Applicants', link: 'http://one.bma.edu.ph/accounting/applicants/overview?_academic=MTA=', icon: 'ellipse' },
                { name: 'Accounts', link: 'http://one.bma.edu.ph/administrator/accounts?_academic=MTA=', icon: 'ellipse' },
                { name: 'Attendance', link: 'http://one.bma.edu.ph/administrator/attendance?_academic=MTA=', icon: 'ellipse' },
                { name: 'Program of Studies', link: 'http://one.bma.edu.ph/registrar/subjects/v2?_academic=MTA=', icon: 'ellipse' },
                { name: 'Teaching Loads', link: 'http://one.bma.edu.ph/registrar/subjects/classes/v2?_academic=MTA=', icon: 'ellipse' },
                { name: 'Section', link: 'http://one.bma.edu.ph/administrator/classes?_academic=MTA=', icon: 'ellipse' },
                { name: 'Setting', link: 'http://one.bma.edu.ph/administrator/setting?_academic=MTA=', icon: 'ellipse' },
                { name: 'Ticketing', link: 'http://one.bma.edu.ph/accounting/ticket?_academic=MTA=', icon: 'ellipse' },
                { name: 'Examination', link: 'http://one.bma.edu.ph/administrator/examination?_academic=MTA=', icon: 'ellipse' },
                { name: 'App Deployment', link: 'http://one.bma.edu.ph/administrator/mobile-applicantion-deployment?_academic=MTA=', icon: 'ellipse' },
                { name: 'Revision Task', link: 'http://one.bma.edu.ph/administrator/task-request?_academic=MTA=', icon: 'ellipse' },
            ],
        }
    },
    methods: {
